  ...
  return (
    <footer className="relative overflow-hidden border-t border-emerald-900 bg-[radial-gradient(circle_at_75%_85%,rgba(132,204,22,0.22),transparent_28%),linear-gradient(135deg,#052e1f_0%,#073b29_45%,#052e1f_100%)] text-white">
      ...
      <div className="border-t border-white/10 px-4 py-6">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 text-center text-sm text-white/60 sm:flex-row">
          ...
          <nav aria-label="Liens légaux" className="flex flex-wrap justify-center gap-x-6 gap-y-2">
            {footerLegalLinks.map((item) => <Link key={item.href} href={item.href} className="transition hover:text-lime-200">{item.label}</Link>)}
          </nav>
        </div>
      </div>
    </footer>
