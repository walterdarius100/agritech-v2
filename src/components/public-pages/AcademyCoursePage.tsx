...
export default async function LocalizedAcademyCoursePage({
  ...
  const copy =
    locale === "en"
      ? {
          ...
          help: "Need help? Contact Agri-tech",
          certification: "Agri-tech certification",
      ...
      : locale === "es"
        ? {
            ...
            help: "¿Necesitas ayuda? Contacta con Agri-tech",
            certification: "Certificación Agri-tech",
            instructor: "Formador",
            program: "Programa de formación",
            access: "Modalidades de acceso",
  ...
  const accessHref = hasAccess
    ? `/academy/cours/${course.slug}/apprendre`
    : checkoutEnabled
      ? checkoutHref
  ...
  return (
    <main className="bg-[#f8faf7]">
      <section className="relative overflow-hidden bg-emerald-950 py-14 text-white sm:py-20">
        ...
        <div
          className="absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(250,204,21,0.20),transparent_24%),linear-gradient(110deg,rgba(2,44,34,0.98)_0%,rgba(2,44,34,0.92)_46%,rgba(6,78,59,0.74)_100%)]"
          aria-hidden="true"
        />
        <Container className="relative z-10">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-4 max-w-4xl text-4xl font-black tracking-tight drop-shadow-sm sm:text-5xl">
              {course.title}
            </h1>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-white/85 drop-shadow-sm">
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-sm font-semibold text-white/90">
              <span className="rounded-full bg-white/10 px-3 py-1 ring-1 ring-white/20 backdrop-blur-sm">
                {course.duration ?? messages.academy.unknownDuration}
              </span>
              <span className="rounded-full bg-white/10 px-3 py-1 ring-1 ring-white/20 backdrop-blur-sm">
                {course.level
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                className="rounded-xl bg-yellow-400 px-5 py-3 font-bold text-emerald-950 shadow-sm transition hover:bg-yellow-300"
                href={accessHref}
              >
                {accessLabel}
              </Link>
              <Link
                className="rounded-xl bg-white/10 px-5 py-3 font-semibold text-white ring-1 ring-white/15 backdrop-blur-sm transition hover:bg-white/15"
                href={academyAccessContactHref}
              >
                {copy.help}
              </Link>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="space-y-8">
            ...
            <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-emerald-100 sm:p-8">
              <div className="grid gap-5 md:grid-cols-[64px_minmax(0,1fr)]">
                <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-yellow-100 text-3xl">
                  🎓
                </span>
                <div>
                  <p className="text-sm font-bold uppercase tracking-[0.18em] text-emerald-700">
                    Validation Agri-tech
                  </p>
                  <h2 className="mt-3 text-3xl font-black text-emerald-950">
                    {copy.certification}
                  </h2>
                  <p className="mt-5 max-w-3xl whitespace-pre-line text-lg leading-8 text-slate-700">
                    {certificationText}
                  </p>
                </div>
              </div>
            ...
            <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-emerald-100 sm:p-8">
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-[96px_minmax(0,1fr)] md:items-start">
                {course.instructor_image_url ? (
                  <span className="relative h-24 w-24 shrink-0 overflow-hidden rounded-3xl">
                    <Image
                      src={course.instructor_image_url}
                      alt={course.instructor_name ?? "Formateur Agri-tech"}
                      fill
                      unoptimized
                      sizes="96px"
                  ...
                  <span className="flex h-24 w-24 shrink-0 items-center justify-center rounded-3xl bg-emerald-50 text-2xl font-black text-emerald-800">
                    AT
                  </span>
                )}
                <div>
                  <p className="text-2xl font-black text-emerald-950">
                    {course.instructor_name || "Équipe Agri-tech"}
                  </p>
                  <p className="mt-1 text-base font-semibold text-emerald-700">
                    {course.instructor_role || "Formateur Academy"}
                  </p>
                  <p className="mt-5 max-w-3xl whitespace-pre-line text-lg leading-8 text-slate-700">
                    {course.instructor_bio ||
                      "Cette formation est préparée par l’équipe Agri-tech dans le cadre de son programme de formation agricole."}
                  </p>
                </div>
              </div>
            </section>
              ...
              <div className="mt-6 space-y-4">
                {modules.map((module, index) => {
                  ...
                  return (
                    <details
                      key={module.id}
                      className="group rounded-3xl bg-white p-5 shadow-sm ring-1 ring-emerald-100 open:ring-emerald-300"
                      open={index === 0}
                    >
                      <summary className="flex cursor-pointer list-none items-start gap-4">
                        <span className="mt-1 text-xl text-emerald-700 transition group-open:rotate-90">
                          ›
                        </span>
                        <span className="min-w-0 flex-1">
                          <span className="block break-words text-lg font-black text-emerald-950">
                            {module.title}
                          </span>
                          <span className="mt-1 block text-sm font-semibold text-slate-500">
                            Module | {moduleLessons.length} leçon
                      ...
                      <ul className="mt-4 space-y-2 text-sm text-slate-700">
                        ...
                        {moduleLessons.length === 0 ? (
                          <li className="rounded-2xl bg-slate-50 px-4 py-3 text-slate-500">
          ...
          <aside className="space-y-6 lg:sticky lg:top-6 lg:h-fit">
            <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-emerald-100">
              ...
              <Link
                className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white transition hover:bg-emerald-800"
                href={accessHref}
              >
                {accessLabel}
              </Link>
