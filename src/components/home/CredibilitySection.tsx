  ...
  return (
    <Section className="bg-transparent">
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        ...
        <div className="grid gap-4 sm:grid-cols-2">{steps.map((step, index) => <div key={step} className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm"><span className="text-sm font-bold text-orange-600">0{index + 1}</span><h3 className="mt-3 font-bold text-emerald-950">{step}</h3></div>)}</div>
      </div>
    </Section>
  );
}
