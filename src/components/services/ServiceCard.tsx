        ...
        <Button
          href={
            locale === "fr"
              ...
              : getLocalizedPath(`/services/${service.slug}`, locale)
          }
          variant="ghost"
          size="sm"
          className="mt-6 w-fit px-0 text-emerald-700 hover:bg-transparent hover:text-emerald-900"
          aria-label={`${messages.services.viewService}: ${service.title}`}
        >
          {messages.services.viewService} →
        </Button>
      </div>
    </Card>
  );
