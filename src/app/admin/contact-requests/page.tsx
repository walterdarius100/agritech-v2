    ...
    <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody>{requests.map((request) => <tr className="border-t border-slate-100" key={request.id}><td className="p-4">{new Date(request.created_at).toLocaleDateString("fr-FR")}</td><td className="font-semibold">{request.full_name}</td><td>{request.email}</td><td>{request.request_type}</td><td>{request.subject ?? "—"}</td><td>{request.status}</td><td>{request.priority}</td><td><Link className="font-semibold text-emerald-800" href={`/admin/contact-requests/${request.id}`}>Voir</Link></td></tr>)}</tbody>
        </table>
