      ...
      <section className="bg-emerald-950 py-14 text-white sm:py-18">
        <Container>
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-yellow-400">Actualités Agri-tech</p>
            <h1 className="mt-5 text-4xl font-bold tracking-tight sm:text-5xl">Articles, analyses et conseils agricoles.</h1>
            <p className="mt-6 text-lg leading-8 text-emerald-50">
              Retrouvez les publications, observations de terrain et contenus pratiques d’Agri-tech pour mieux comprendre les réalités agricoles en Haïti.
            </p>
            <div className="mt-8">
              <Button href="/contact" variant="outline" size="lg" className="border-white/70 text-white hover:bg-white/10">
                Parler à Agri-tech
              </Button>
            </div>
          </div>
          ...
          <article className="overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-sm ring-1 ring-slate-100 lg:grid lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="flex flex-col justify-center p-6 sm:p-7 lg:p-8">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-800">
                {featuredArticle.category ?? "Article"} · {formatArticleDate(featuredArticle.published_at)}
              </p>
              <h2 className="mt-4 text-2xl font-bold tracking-tight text-emerald-950 sm:text-3xl">{featuredArticle.title}</h2>
              {featuredArticle.excerpt ? <p className="mt-4 text-base leading-7 text-slate-600">{featuredArticle.excerpt}</p> : null}
